...
function plot_connected_neurons(connectivity, config, post::Symbol=:Exc, n0::Int=rand(1:20))
    ...
    plots = map(zip([:Exc, :PV, :SST], [:darkred, :blue, :darkorange])) do (pre, c)
        ...
        plot!(title = string("$pre => $post"), xlims=xlims, ylims=ylims , frame=:axes, margin=10Plots.mm, xticks=ticks, yticks=ticks)
    end
...
function create_connection_histogram(connectivity, config, pre=:Exc, post=:Exc)
    ...
    heatmap(range(0, 1,bins), range(0,1,bins),  counts', xlabel="Distance", ylabel="Distance", title="", color=:viridis, cbar=false,)
end
