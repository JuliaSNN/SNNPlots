...
function plot_connected_neurons(network, config, post::Symbol=:Exc, n0::Int=rand(1:20))
    ...
    plots = map(zip([:Exc, :PV, :SST], [:darkred, :blue, :darkorange])) do (pre, c)
        d = []
        w = []
        for n in 1:length(points[pre])
            push!(d, periodic_distance(points[post][n0], points[pre][n], grid_size))
            push!(w, links[name(pre,post)][n0,n])
        end

        xlims = extrema([getfield(points, post)[i][1] for i in eachindex(getfield(points, post))])
        ylims = extrema([getfield(points, post)[i][2] for i in eachindex(getfield(points, post))])
        x = [points[pre][i][1] for i in 1:length(points[pre])]
        y = [points[pre][i][2] for i in 1:length(points[pre])]
        x0 = points[post][n0][1]
        y0 = points[post][n0][2]
        scatter(x,y, xlabel="X", ylabel="Y", title="", c=:grey, ms=2, msc=:grey, label="", alpha=0.4)
        n = findall(x->x==1, w)
        scatter!(x[n], y[n], c=c, ms=5, msc=c, label="")
        scatter!([x0],[y0], xlabel="Tonotopic axis (mm)", ylabel="Isofrequency axis (mm)", title="Connected neurons", c=:black, ms=8, msc=:grey, label="")
        plot!(title = string("Connected to $post from ", pre), xlims=xlims, ylims=ylims , frame=:axes, margin=10Plots.mm)
    end
    p1 = plot(plots..., size=(1200,400), layout=(1,3))


    plots = map([:Exc, :PV, :SST]) do pre
        _m = maximum([maximum(sum(links[name(pre,post)], dims=2)[:,1]),3])
        p = create_connection_histogram(network, config, pre, post)
...
function count_neurons(pre::Symbol, post::Symbol, network, config, samples =50)
    ...
    N_s = ϵ * connections[name(pre,post)].p * Npop[pre] * (1-p_long[pre]) |> x-> round(Int,x)
    N_l = ϵ * connections[name(pre, post)].p * Npop[pre] * (p_long[pre]) |> x-> round(Int,x)
    @info "$pre => $post (real/expected):  Short ($_Ns/$(N_s));  Long ($(_Nl)/$(N_l))"
    return N_s, N_l
end

# Function to create a 2D histogram of connection probabilities
function create_connection_histogram(network, config, pre=:Exc, post=:Exc)
    ...
    for j in eachindex(pre_points)
        for i in findall(links[name(pre, post)][:, j])
    ...
    heatmap(range(0, 1,bins), range(0,1,bins),  counts', xlabel="Distance", ylabel="Distance", title="Connection Probability", color=:viridis, cbar=false)
end
...
function animate_raster(points, model, interval, path, fps=10)
    ...
    anim = @animate for t in interval
        plot(
            xlim=(0, 1), ylim=(0, 1), size=(800, 800),
            legend=:none, title="t = $t ms"
        )
        ...
        begin
            for delay in 1:2:10
                ...
                scatter!(
                    ...
                    color=c, msc=c, markersize=15, alpha=0.3/log(delay+1)
                )
                c = :orange
        ...
        begin
            for delay in 1:2:30
                ...
                scatter!(
                    ...
                    color=c, msc=c, markersize=5/(delay+0.1), alpha=1/(delay+0.1)
                )
            end
